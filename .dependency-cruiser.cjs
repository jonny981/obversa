...
module.exports = {
  ...
  forbidden: [
    ...
    {
      name: 'api-reaches-no-package',
      comment: '@obversa/api is a contract package; it reaches no other package or plugin',
      severity: 'error',
      from: { path: '^packages/api/' },
      to: { path: '^(packages|plugins)/', pathNot: '^packages/api/' },
    },
    {
      name: 'core-reaches-api-only',
      comment: '@obversa/core uses the API contracts but no runtime or plugin',
      severity: 'error',
    ...
    },
    {
      ...
      comment: 'the runtime reaches API contracts and core execution; never a provider plugin or surface',
      severity: 'error',
      from: { path: '^packages/runtime/' },
      to: { path: '^(packages|plugins)/', pathNot: '^packages/(runtime|api|core)/' },
    },
    {
      name: 'runner-reaches-runtime-and-engine-only',
      comment: 'the runner supervises through public runtime, API and core entries; it never reaches an adapter',
      severity: 'error',
    ...
    },
    {
      ...
      to: { path: '^(packages|plugins)/', pathNot: '^packages/(builtin-workflows|runtime|api)/' },
    },
    {
      name: 'memory-plugin-reaches-memory-only',
      comment: 'a memory plugin reaches API contracts and core execution only',
      severity: 'error',
    ...
    },
    {
      ...
      comment: 'the Agent SDK plugin reaches API contracts and core execution only',
      severity: 'error',
