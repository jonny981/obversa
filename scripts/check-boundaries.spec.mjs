...
const SOURCE = "/repo/packages/surface-diff/tsconfig.json";
...
test("a package.json imports alias, a workspace devDependency, and an alias that installs a sibling under another name are arrows the rules must allow", () => {
  ...
  assert.deepEqual(
    manifestImportTargets({ imports: { "#kit": { node: "@obversa/surface-decision/client", default: "./src/local.mjs" }, "#mem": ["../memory/src/index.ts", "./fallback.mjs"] } }, at),
    ["@obversa/surface-decision", "@obversa/memory"],
  ...
  assert.deepEqual(
    internalDependencies({ dependencies: { hidden: "npm:@obversa/surface-decision@0.1.0", other: "workspace:@obversa/memory@*", "@obversa/runtime": "workspace:^", ext: "npm:lodash@4", plain: "^1.0.0" } }, "dependencies"),
    ["@obversa/memory", "@obversa/runtime", "@obversa/surface-decision"],
  ...
  assert.deepEqual(internalDependencies({ dependencies: { hidden: "workspace:../surfacer" } }, "dependencies", at), ["@obversa/surface-decision"], "a relative workspace alias is placed by its directory");
...
test("a dependency value is a registry range, a registry alias, a workspace package, or refused", () => {
  ...
  assert.deepEqual(dependencyTarget("hidden", "workspace:../surfacer", at), { name: "@obversa/surface-decision" });
...
test("a manifest's entry fields are placed by the package their real path lies in", () => {
  ...
  assert.deepEqual(manifestPathTargets({ main: "../surfacer/src/index.mjs" }, at), ["@obversa/surface-decision"], "main into a sibling");
  assert.deepEqual(manifestPathTargets({ module: "../memory/src/index.ts", browser: { "./x.js": "../surfacer/src/client.mjs", fs: "browserify-fs" } }, at), ["@obversa/memory", "@obversa/surface-decision"], "module and a browser map; a bare package name is not a path");
  assert.deepEqual(manifestPathTargets({ types: "../surfacer/types/index.d.ts" }, at), ["@obversa/surface-decision"], "types");
  assert.deepEqual(manifestPathTargets({ typings: "../surfacer/types/index.d.ts" }, at), ["@obversa/surface-decision"], "typings");
  ...
  assert.deepEqual(manifestPathTargets({ typesVersions: { "*": { "*": ["types/*"] } } }, at), [refusal("typesVersions uses a wildcard, which substitutes a consumer subpath the scan cannot bound")], "a wildcard mapping lets @obversa/surface-diff/../../surfacer/index resolve into the sibling under the compiler");
  ...
  assert.deepEqual(manifestPathTargets({ exports: { "./deep": "../surfacer/src/index.mjs" } }, at), ["@obversa/surface-decision"], "an exports leaf");
  assert.deepEqual(manifestPathTargets({ directories: { lib: "../surfacer/src" } }, at), ["@obversa/surface-decision"]);
  assert.deepEqual(manifestPathTargets({ publishConfig: { access: "public", directory: "../surfacer" } }, at), ["@obversa/surface-decision"], "publishing another directory");
  ...
  assert.deepEqual(manifestPathTargets({ main: "./x?/../../surfacer/index.cjs" }, at), ["@obversa/surface-decision"], "a question mark does not end the path");
  assert.deepEqual(manifestPathTargets({ module: "./x*/../../memory/index.js", types: "./t?/../../surfacer/index.d.ts" }, at), ["@obversa/memory", "@obversa/surface-decision"]);
  assert.deepEqual(manifestImportTargets({ imports: { "#x": "./x?/../../surfacer/src/index.mjs" } }, { file: "/repo/packages/surface-diff/package.json", root: "/repo" }), ["@obversa/surface-decision"], "an imports leaf too");
});
...
test("a project config is read as the compiler reads it: every extends form, ${configDir}, and the options a base carries", () => {
  ...
  const chain = { "/repo/packages/surface-diff/base.json": '{ "extends": "./deeper.json", "compilerOptions": { "jsxImportSource": "@obversa/surface-decision" } }', "/repo/packages/surface-diff/deeper.json": '{ "compilerOptions": { "paths": { "#m/*": ["../memory/src/*"] } } }' };
  assert.deepEqual(deps('{ "extends": "./base.json" }', chain), ["@obversa/surface-decision", "@obversa/memory"], "a same-package base carries the dependency, and its own base too");
  assert.deepEqual(deps('{ "extends": "/repo/packages/surface-diff/base.json" }', chain), ["@obversa/surface-decision", "@obversa/memory"], "an absolute parent");
  assert.deepEqual(deps('{ "extends": "./base" }', chain), ["@obversa/surface-decision", "@obversa/memory"], "an extensionless parent is probed with .json, as the compiler does");
  assert.deepEqual(deps('{ "extends": ["./deeper.json", "./base.json"] }', chain), ["@obversa/surface-decision", "@obversa/memory"], "an extends array inherits each base");
  assert.deepEqual(
  ...
  assert.deepEqual(deps('{ "extends": "../surfacer/tsconfig.json" }', { "/repo/packages/surface-decision/tsconfig.json": deeper, "/repo/packages/surface-decision/deeper.json": "{}" }), ["@obversa/surface-decision", "@obversa/surface-decision", "@obversa/memory"], "a base in a sibling is a crossing, its own base too, and what it carries is inherited");
  ...
  assert.deepEqual(deps('{ "compilerOptions": { "baseUrl": "${configDir}/../surfacer" } }'), ["@obversa/surface-decision"], "${configDir} expands to the config's directory");
  ...
  assert.equal(isProjectConfig("packages/surface-diff/config/tsconfig.build.json", "{}"), true, "a nested tsconfig");
  assert.equal(isProjectConfig("packages/surface-diff/jsconfig.json", "{}"), true, "a jsconfig");
  assert.equal(isProjectConfig("packages/surface-diff/config/build.json", '{ "compilerOptions": { "paths": {} } }'), true, "any name, by its project fields: tsc -p accepts it");
  assert.equal(isProjectConfig("packages/surface-diff/config/build.json", '{ "extends": "./x.json" }'), true);
  assert.equal(isProjectConfig("packages/surface-diff/data.json", '{ "a": 1 }'), false, "plain data");
  assert.equal(isProjectConfig("packages/surface-diff/list.json", "[1]"), false);
  assert.equal(isProjectConfig("packages/surface-diff/package.json", '{ "files": ["dist"] }'), false, "the manifest is read separately");
...
test("every effective option that can reach a sibling is placed by the package it lands in; a value that reaches every package is refused", () => {
  ...
  assert.deepEqual(deps('{ "compilerOptions": { "typeRoots": ["./types", "../surfacer/types"] } }'), ["@obversa/surface-decision"], "typeRoots");
  assert.deepEqual(deps('{ "compilerOptions": { "typeRoots": ["../../hosts/types"], "types": ["../surfacer/types"] } }'), ["@obversa/surface-decision"], "a types entry by path");
  assert.deepEqual(deps('{ "compilerOptions": { "baseUrl": "../surfacer" } }'), ["@obversa/surface-decision"], "baseUrl");
  ...
  assert.deepEqual(deps('{ "compilerOptions": { "paths": { "#kit/*": ["../surfacer/src/*"] } } }'), ["@obversa/surface-decision"], "a paths alias into a sibling");
  assert.deepEqual(deps('{ "references": [{ "path": "../memory" }] }'), ["@obversa/memory"], "a project reference");
  assert.deepEqual(deps('{ "include": ["src/**/*", "../surfacer/src/**/*"] }'), ["@obversa/surface-decision", "@obversa/surface-decision"], "include with a glob into a sibling: the file it matches, and the directory it walks");
  ...
  assert.deepEqual(deps('{ "compilerOptions": { "baseUrl": "./src", "paths": { "#surf/*": ["../../surfacer/src/*"] } } }'), ["@obversa/surface-decision"], "a paths target relative to an own baseUrl");
  ...
  assert.equal(resolved("#pick/surfacer", { paths: { "#pick/*": ["../*/src/index.ts"] }, pathsBasePath: "/repo/packages/surface-diff" }), "/repo/packages/surface-decision/src/index.ts", "a wildcard in a directory segment ranges over siblings");
...
test("the guard, run on a disposable copy of the tree, refuses a shipped host import of a file the walk never scans", { timeout: 300_000 }, () => {
  ...
  try {
    ...
    writeFileSync(nestedHost, JSON.stringify({ name: "@obversa/surface-diff", exports: { "./escape": "./dist/escape.mjs" } }));
...
test("the loader-hatch scan covers package and plugin TypeScript", () => {
  ...
  assert.equal(scansImports("plugins/engine-codex-cli/src/index.ts"), true);
