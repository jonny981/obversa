...
function delay(ms, signal) {
  return /** @type {Promise<void>} */ (new Promise((resolve, reject) => {
    let timer;
    const abort = () => { clearTimeout(timer); reject(new Error("browser budget phase aborted")); };
...
async function devtools(profile, origin) {
  const port = await BROWSER_RENDER_CHAIN.run("DevTools port", async (signal) => {
    const portFile = path.join(profile, "DevToolsActivePort");
    const started = Date.now();
    while (!existsSync(portFile)) await delay(100, signal);
    console.log(`browser DevTools port wait: ${Date.now() - started}ms`);
    return Number(readFileSync(portFile, "utf8").split("\n")[0]);
  });
  const page = await BROWSER_RENDER_CHAIN.run("DevTools target", async (signal) => {
    while (true) {
      const fetchStarted = Date.now();
      try {
        const fetchSignal = AbortSignal.any([signal, AbortSignal.timeout(DEVTOOLS_FETCH_TIMEOUT_MS)]);
        const targets = /** @type {any} */ (await (await fetch(`http://127.0.0.1:${port}/json/list`, { signal: fetchSignal })).json());
        const page = targets.find((target) => target.type === "page" && target.url.startsWith(origin));
        if (page) {
          console.log(`browser DevTools target fetch: ${Date.now() - fetchStarted}ms`);
          return page;
        }
      } catch (error) {
  ...
  await BROWSER_RENDER_CHAIN.run("DevTools handshake", (signal) => new Promise((resolve, reject) => {
    ...
    const onClose = () => finish(reject, new Error("DevTools socket closed before the handshake"));
    const onAbort = () => { ws.close(); finish(reject, new Error("DevTools handshake budget expired")); };
  ...
  ws.addEventListener("error", () => failPending(new Error("DevTools socket failed during a command")));
  ws.addEventListener("close", () => failPending(new Error("DevTools socket closed during a command")));
  const send = (method, params = {}, signal) => new Promise((resolve, reject) => {
    if (socketFailure) return reject(socketFailure);
    if (signal?.aborted) return reject(new Error(`DevTools command ${method} was aborted`));
    const id = ++seq;
    const abort = () => { pending.delete(id); reject(new Error(`DevTools command ${method} was aborted`)); };
    const finish = (callback) => (value) => {
      signal?.removeEventListener("abort", abort);
      callback(value);
    };
  ...
  const evaluate = async (expression, signal) => {
    const { result, exceptionDetails } = await send("Runtime.evaluate", { expression, returnByValue: true }, signal);
  ...
  const press = async (key, keyCode, text, signal) => {
    const base = { key, code: key, windowsVirtualKeyCode: keyCode, nativeVirtualKeyCode: keyCode };
    await send("Input.dispatchKeyEvent", { type: text ? "keyDown" : "rawKeyDown", ...base, ...(text ? { text, unmodifiedText: text } : {}) }, signal);
    await send("Input.dispatchKeyEvent", { type: "keyUp", ...base }, signal);
  };
  return {
    send,
    evaluate,
    tab: (signal) => press("Tab", 9, undefined, signal),
    enter: (signal) => press("Enter", 13, "\r", signal),
    type: (text, signal) => send("Input.insertText", { text }, signal),
    close: () => { failPending(new Error("DevTools session closed")); ws.close(); },
  };
}
...
async function keyboardRoute(dt, signal) {
  await dt.send("Emulation.setFocusEmulationEnabled", { enabled: true }, signal);
  const active = (expression) => dt.evaluate(`(() => { const a = document.activeElement; ${expression} })()`, signal);
  let tabs = 0;
  let reached = null;
  ...
  await dt.tab(signal); // Save
  await dt.tab(signal); // Discard
  ...
  await dt.enter(signal); // the opener holds focus again: reopen
  await dt.type("Looks wrong", signal);
  await dt.tab(signal); // Save
  ...
  const returned = await dt.evaluate(`(() => { const b = [...document.querySelectorAll("button")].find((x) => /^Return 1 annotation/.test(x.textContent)); if (!b) return null; b.click(); return b.textContent; })()`, signal);
...
test("the review surface renders under the exact CSP with zero violations and file-scoped go-to-source", { skip: CHROME ? false : "Google Chrome is not installed", timeout: BROWSER_TEST_TIMEOUT_MS }, async () => {
  ...
  try {
    ...
    try {
      keys = await BROWSER_RENDER_CHAIN.run("keyboard route", (signal) => keyboardRoute(dt, signal));
    } finally { dt.close(); }
    const submittedStarted = Date.now();
    const submittedAndAcked = await BROWSER_RENDER_CHAIN.run("submission and acknowledgement", async () => {
    ...
    keys.payload = submittedAndAcked.payload;
    keys.ack = submittedAndAcked.ack;
  } finally {
    await BROWSER_RENDER_CHAIN.run("cleanup", async () => {
      chrome?.kill("SIGKILL");
