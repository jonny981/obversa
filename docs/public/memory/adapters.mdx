...
The runtime accepts the `Memory` port and imports no adapter itself. Your
program selects and creates the adapter.
...
The adapter stores each scope in a private Git reference. It doesn't change
the current branch, the index, or the worktree.

Delete and eviction remove a path from that private reference. The older Git
object can stay in the repository until Git removes unused objects. Git
mirror clones, mirror pushes, and bundles made with `--all` copy the private
reference and its plain-text contents, and backups can keep it too. Don't
treat the reference as local-only, and don't treat deletion as immediate,
secure erasure.
