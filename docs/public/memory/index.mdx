...
`@obversa/api` defines one storage-neutral memory port. The port has a
...
Adapters use the same error codes. A conformance kit checks this contract
...
`rename` moves a file or a complete directory. It doesn't replace an existing
destination.
...
`@obversa/memory-simple` and `@obversa/memory-git` use these default limits:

- One file can contain 65,536 bytes.
- One scope can contain 1,048,576 bytes.
- One scope can contain 256 files.

Both accept `.txt`, `.md`, `.json`, `.py`, `.yaml`, and `.yml` files.

When a write takes a scope past a limit, the adapter removes files in order of
earliest write time until the write fits. A read doesn't change a file's write
time. The adapter never removes the file being written.
...
The memory root can't be a file, and you can't delete or rename it.
...
A failed result holds the command, the error code, and a message. Some errors
also carry the path and structured details.
