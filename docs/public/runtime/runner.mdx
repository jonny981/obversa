...
The original wall-clock deadline includes time spent paused. Logical dispatches
and recorded worker restarts also keep their counts. A pause does not restore
spent budgets; resuming after the deadline returns `TIMEOUT` without a worker.
