...
description: "One step of a run executes on a remote machine while the orchestrator and the record stay on yours, and the change comes back as a commit or a patch."
---
...
A run is a graph of steps, and the steps aren't alike. Most of them are
cheap: a command that runs your tests, a condition that reads a file, a
question that waits for a person. One or two aren't: the step that builds
...
on another machine. The orchestrator stays where you started it, and the
event record is written where it always was. The change the step made
comes back as a commit or a patch, not as a filesystem you have to keep in
sync.

## What stays local

The parts that make a run inspectable don't move.
...
until it doesn't: a partial write during a network stall leaves a
workspace nobody can explain afterwards. A commit is atomic, it carries
its own description, and it's the artefact the rest of this runtime
already records against. The step either produced one or it didn't.

## What it does not do

It doesn't distribute a run. The graph isn't sharded across machines, and
steps don't migrate for load. One named step is given a place to execute,
...
record, rather than quietly carrying on somewhere else.
