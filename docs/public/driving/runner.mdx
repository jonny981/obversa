...
description: "Run a graph in a worker process that checks its engines before any work, pauses on a durable record when an engine isn't ready yet, resumes from that exact pause, and recovers after a crash without repeating finished work."
---

Start a graph in a worker, and let the calling process be its watchdog.
The worker can check every engine seat before the first step runs. The
watchdog waits for the worker, stops its child processes, and restarts the
worker within the time and restart limits you declare. Every step is
written to the run's record, so a killed run carries on from where it
stopped. A check that fails on something that clears, such as a rate
limit, pauses the run on a record you can resume from later.
...
engine that isn't ready. The run pauses before any work is dispatched.
The file then makes the engine ready, stops the old watchdog, resumes the
run from the recorded pause, and prints what happened. The engine and its
bindings live in `examples/preflight-host.mjs`. The file copies it into the
disposable repository it creates, and the runner loads that copy as the
host module.
...
- **A static check** asks the engine to admit the seat, with the node's
  real configuration and no prompt. The engine reports the identity it
  will run under (adapter, provider, model family, model, executable), and
  the runtime compares it with the saved identity. An engine that doesn't
...
- **A failed check that retires nothing pauses the run** on a
  `PREFLIGHT_PAUSED` record before any dispatch. The record carries the
  pause event id. Nothing has run, so there's nothing to reconcile.
- **Resume takes that exact id.** Pass `preflightEventId` to
  `resumeSupervisedRun`. The static checks run again, and the live
...
- **A lane with no admissible seat ends the run.** When every target of a
  ...
  `PREFLIGHT_FAILED` and `done` resolves to `fail` with that code. It's
  terminal: nothing was dispatched and there's no pause to resume from.
...
If the pause event itself can't be written, the run returns a failure
...
exhausted credit or an exhausted quota retire that provider and model, and
also the provider and model the engine reported, if they differ. A missing
command-line tool or an invalid configuration retire the adapter. A rate
limit or a transport error retire nothing, because they clear in seconds
or minutes. A quota is an allowance gone for hours or longer. An old auth
record with no provider recovers one from its lane. When that's ambiguous,
the run refuses before any work with `ENGINE_IDENTITY_UNRESOLVED`.
...
`StorageError` with `STORAGE_LIMIT_EXCEEDED`, and a match on a configured
known secret raises `KNOWN_SECRET`. Inside the worker, the refused write
...
node bindings and the engine bindings. `examples/preflight-host.mjs` is
one.
...
start or resume call. The runner copies the value when it's set, and
stores neither the name nor the value in the run. `resolvedInputs` is
durable storage, so keep secrets out of it.
...
A saved start without a saved result doesn't prove whether an outward
...
the run id and the exact `position` from its `graph:node-paused` event.
For a preflight pause, pass the run id and the `preflightEventId`. The
definition, host module and limits come from the stored run. Resume writes
no second start event and repeats no finished occurrence.
...
`WORKSPACE_ANCHOR_INVALID`. If the snapshot can't be captured at pause
time, the run fails with `WORKSPACE_ANCHOR_WRITE` and no resumable pause
is recorded. If it was captured but can't be stored, the run pauses with
the same code, and resume compares the workspace with the captured
snapshot's digest. A capture that fails during that comparison is a
capture failure, not evidence of an edit.

The host's action policy runs again on resume. Record approval where that
policy reads it. Calling resume isn't an approval.
...
- **Elapsed time:** `limits.timeoutMs` covers worker execution and restart
  ...
  launch. Resume checks and preflight checks before launch don't spend it.
  Engine checks inside the worker do.
...
Usage is reported per engine call. Missing usage isn't zero: a call
without a receipt counts as unknown, and a node's usage is `partial` when
some of its calls have receipts and some don't. The check calls before the
run report their own usage, separate from the nodes'.
...
inherited marker. Elsewhere it follows the observed process tree, so a
helper that moves into a new session before it's observed isn't swept.
...
watchdog itself dies, its lock and lease remain, and a new watchdog doesn't
force a takeover.
