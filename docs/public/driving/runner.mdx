...
description: "If the worker dies, a new one reads the record and a finished step does not run again. Engines are checked before the first step."
---
...
[runtime](/concepts/running) is enough.
...
```ts examples/preflight-supervised-run.ts (excerpt) {2-4,13-15}
...
```ts examples/preflight-supervised-run.ts (excerpt) {3,11}
  await handle.stop();
  handle = undefined;
...
```ts examples/preflight-supervised-run.ts
...
- [Runner](/packages/runner): the package's public entry points.
- [The record](/concepts/record): what a resumed run skips, repeats and
  asks a person to reconcile.
- [Watch a run in the browser](/driving/monitor): the page a supervised run
