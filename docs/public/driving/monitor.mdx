...
Turn the page on and the run binds a free port on the loopback address. It
folds the run's own events into the state of each declared step, and
serves that state as a page and as JSON. Nothing on the page starts, stops
or edits the run. Its one control answers a question the run is waiting
on, through the same callbacks client any router uses.
...
The address is written once as a `monitor` event, so it's in the record
and in any `onEvent` sink. The run never prints it. `result.monitor`
carries the same `url` and a `close()`.

## When it is on

- `monitor: true` turns it on for a plain `run`. The default is off.
- Under `supervise` it's on unless you pass `monitor: false`.
...
The server doesn't keep the process alive. A script that finishes its run
exits as it would without the page, and the page goes with it. A process
that stays up keeps serving the final state until `close()` is called.

Call `result.monitor.close()` even when the run never started, because the
page still binds the port. If the environment fails to come up, the run
fails and the page shows that final state.
...
Any other route is 404, any other method is 405, and a body that isn't
JSON is 400. A request whose `Host` isn't the bound address is refused.
`/answer` takes only `application/json`, so a page from another origin
can't post to it.

## Where to go

See [Supervised local runs](/driving/runner) for a run that survives a
crash, and [how a run is recorded](/recording/how-a-run-is-recorded) for
the record the page is folded from.
