...
| It runs all day. | A record of every step as it happens. A workflow started again with `resume: true` carries on from that record, and the [supervised runner](/driving/runner) restarts a killed run from its own record. |
