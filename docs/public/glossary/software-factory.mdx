...
pull requests out, agents doing the work in between. In practice it's a
process: one agent writes, another reviews, the work goes back when the
review fails, tests run as commands, and a person decides at the end.
...
Sending work back is what makes a review count. A pipeline that never sends
work back is a queue.

A product factory is the same picture with a larger scope. The file has the
same shape.

## Where to go

[Software factory, as a workflow](/use-cases/software-factory) covers the
use case.
[What is an agent pipeline with a review and a person?](/glossary/agent-pipeline)
shows the smallest version of the shape.
