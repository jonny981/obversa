...
The record outlives the run. A run that records writes every step as a line
in a file you can read afterwards.
[How a run is recorded](/recording/how-a-run-is-recorded) shows what the
file holds.
...
You declare that reviewer in the outer layer. The workflow builder refuses
one from the writer's own family.

## Where to go

[What is an agent harness?](/glossary/agent-harness) covers the inner
layer. [Domain-specific harnesses](/use-cases/domain-specific-harnesses)
shows a whole job written this way.
