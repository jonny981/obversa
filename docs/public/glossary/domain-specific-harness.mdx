...
same one everyone has. The harness is the part that's yours, and most of
it is process: what runs first, what checks it, what happens when the check
fails, and who signs at the end.

Products built on a model end up owning the process around it. In code,
that process is a workflow. The file below is
...
has its own inner loop, which
[What is an agent harness?](/glossary/agent-harness) covers.

The person is part of the harness. Write their decision in as a role, as
the file does with `approve`. The run pauses at that stage until the answer
arrives through the callbacks client.

Use the record as your audit. A run that records writes every step as it
happens, so you can read it back later.
[How a run is recorded](/recording/how-a-run-is-recorded) shows what a
record holds.

## Where to go

[Domain-specific harnesses](/use-cases/domain-specific-harnesses) covers the
use case in full. The [feature team](/workflows/feature-team) page shows a
real run.
