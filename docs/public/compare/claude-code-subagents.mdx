...
For structured assessments rather than workspace coding, install the
[Jev engine](/packages/engine-jev-api) separately from the Obversa bundle.
It uses workspace mode `none` and answers questions over recorded state;
your workflow chooses how to act on those answers.
...
| The workers | Claude models, inside Claude Code. | CLI coding engines such as Claude Code, Codex, Grok and OpenCode run one fresh process per call. |
...
| A run that survives a crash | Transcripts on disk; a subagent can be resumed. | A run records to a file; a workflow started again with `resume: true` carries on from that record and repeats nothing that finished. The supervised runner restarts a compiled graph from its own record. |
