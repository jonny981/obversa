...
description: "Claude Code subagents split one session's work across specialised Claude assistants. Obversa drives Claude Code, Codex, Grok and OpenCode as a team from a TypeScript file, with feedback loops and a person at the gate. What each is for, and where each wins."
---

Choose by where the work lives. Subagents fit when you're inside one
Claude Code session and want to hand parts of a task to specialised
assistants with their own tools and prompts. Obversa fits when the work
needs more than one family of model, a review that sends work back with
findings, a person who decides, and a record you can read after the run.

In Claude Code's own words: "Subagents are specialized AI assistants that
...
session and can declare its own permission mode, which the session's mode
can override. Its transcript is kept on disk, so it can be resumed. The
model is one of the Claude family.
...
| The workers | Claude models, inside Claude Code. | Engines such as Claude Code, Codex, Grok and OpenCode, with a fresh process for every call, or an API engine. |
| A review that sends work back | A subagent can be asked to review. Sending work back is direction you give. | Built in: a review sends the work back to the stage that owns it, with the findings, up to a limit you set. |
...
| A run that survives a crash | Transcripts on disk. A subagent can be resumed. | A run can record to a file. Started again with `resume: true`, a workflow carries on from that record and repeats nothing that finished. The supervised runner restarts a compiled graph from its own record. |
...
- You're already in Claude Code and the task is one session's work.
...
- A review should come from a different family than the one that wrote the
  work.
- The process, reviews included, should be a file under version control,
  with the run's record beside it.
