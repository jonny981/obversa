...
| The workers | Your code in activities, which may call any model. | CLI coding engines such as Claude Code, Codex, Grok and OpenCode run one fresh process per call. |
...
| A run that survives a crash | Event history and replay; the run continues from the exact point. | A run records to a file; a workflow started again with `resume: true` carries on from that record and repeats nothing that finished. The supervised runner restarts a compiled graph from its own record. |
