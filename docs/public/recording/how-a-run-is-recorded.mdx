...
description: "Every step appends events to a file as it happens, and that record is why a killed run restarts where it stopped."
---
...
Run the offline review workflow, then open the file it wrote. Each line is
...
head -n 8 .obversa/records/offline-review.jsonl
```

```text
{"kind":"loop:start","ts":1789042145568,"path":["write-config"],"depth":1,"max":4}
{"kind":"loop:iteration","ts":1789042145569,"path":["write-config"],"iteration":1}
{"kind":"job:start","ts":1789042145569,"path":["write-config"],"label":"author"}
{"kind":"job:end","ts":1789042145569,"path":["write-config"],"label":"author","outcome":{"status":"pass","summary":"wrote the base config"}}
{"kind":"loop:condition","ts":1789042145569,"path":["write-config"],"which":"until","iteration":1,"result":{"met":true,"reason":"a draft exists: true"}}
{"kind":"job:start","ts":1789042145569,"path":["write-config"],"label":"review"}
{"kind":"job:end","ts":1789042145569,"path":["write-config"],"label":"review","outcome":{"status":"fail","summary":"Missing a request timeout.","revision":{"reason":"Missing a request timeout.","findings":[{"reviewer":"correctness","evidence":"No timeout set; a hung upstream call blocks forever."}]}}}
{"kind":"loop:review","ts":1789042145569,"path":["write-config"],"outcome":{"status":"fail","summary":"Missing a request timeout.","revision":{"reason":"Missing a request timeout.","findings":[{"reviewer":"correctness","evidence":"No timeout set; a hung upstream call blocks forever."}]}},"accepted":true}
```
