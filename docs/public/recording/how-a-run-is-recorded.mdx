...
stage that finished is skipped. A stage that failed runs again. A gate still
waiting for a person is observed, and the run exits with the recorded pause,
so a run started on a schedule asks once and never twice when it reads
the same store of pending questions. A run given a fresh in-memory
