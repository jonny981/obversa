...
of it. The runtime records what was requested, what actually ran, what came
back, and whether token usage was reported or unknown.

An engine-backed node attempt is one fresh engine call. Command adapters use
one fresh CLI process, and data-only attempts use no engine process. The
graph executor connects attempts to graph dispatch.

## Run the example


`examples/safe-node-attempt.ts` runs the Grok and OpenCode adapters against
local scripted executables. It doesn't call a model or the network. Grok
returns a native structured result. OpenCode returns one marked text part,
which the job's own parser reads. Both adapter results pass through the
public result validator before their final parts are used. OpenCode
deliberately leaves out a valid usage receipt, so the runtime keeps its
usage as `unknown` instead of zero.

The report prints the stub executable paths relative to the temporary
directory. The exported `attemptReport` keeps the absolute paths from the
adapter results.
...
different model, the two records stay separate and the change stays visible.
...
The workspace mode is a ceiling. An approval never adds a capability the mode
withholds, and a bypass never goes past it. A reviewer with no way to read
the work is refused rather than run. A review can rest only on material the
seat could open.
...
The grace doesn't start more model work.
...
wait can't turn the attempt into a success.
...
records the attempt. Hosts own two things themselves: recovery after the
supervising process dies, and checks against the CLIs actually installed.
