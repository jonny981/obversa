...
- **`startSupervisedRun`:** start a stored graph with a watchdog in the calling process.
...
- **Run types:** `SupervisedRunOptions` describes the stored run, host module, workspace, and limits. `SupervisedRunStatus` describes progress, cleanup scope, and retained workspace ownership.
