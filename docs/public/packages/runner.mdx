...
description: "Supervise a graph in a worker process: check its engines before the first step, pause on a record when an engine isn't ready yet, resume from that pause, and recover after a crash."
---

Run a stored graph in a worker, with a watchdog in your own process. When
the plan asks for engine checks, the worker checks every engine seat before
the first step. A check that fails on something that clears, such as a rate
limit, records a pause, and you can resume the run from that exact pause.

The watchdog restarts a crashed worker within your limits and never repeats
work that finished. If the watchdog itself dies, decide by hand what happens
next. Its lock and lease stay until you do.
...
This file starts a one-node graph whose engine isn't ready, watches the
...
Put the host module you pass as `module` inside `runRoot`, the worker root,
and export `bindRun` from it. The runner loads it, never the runtime. It's
pinned by byte digest for the life of the run.
...
The [supervised local runs](/driving/runner) page shows what it printed and
what each of the five engine calls was.

## Public entry points

| export | what it does |
| --- | --- |
| `startSupervisedRun` | Starts a stored graph in a worker with a watchdog in the calling process and returns a handle. Engine checks inside the worker spend its `limits.timeoutMs`. Checks before launch don't. |
| `resumeSupervisedRun` | Reopens one recorded pause: a node pause by its `position`, or a preflight pause by its `preflightEventId`. |
...
| `SupervisedRunOptions`, `ResumeSupervisedRunOptions`, `ResumePreflightSupervisedRunOptions`, `SupervisedRunStatus`, `SupervisedRunUsage` | The option and status types. The preflight resume options name the pause by run id and `preflightEventId`, with no `position`. `SupervisedRunUsage` reports a call without a receipt as unknown, not zero. |

## Source

`packages/runner` in the repository. It depends on the public
`@obversa/runtime`, `@obversa/api` and `@obversa/core` APIs.
