...
| `@obversa/engine/testing` | `runEngineConformance` for the run contract, `runEngineAdmissionConformance` for `admit`, and `MockEngine`. An adapter author runs both kits; an adapter without `admit` runs the first alone. |
