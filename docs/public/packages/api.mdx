...
typed failure. This package holds that contract and the memory port, which
the runtime and plugins share. It also defines and checks graph plans,
stored events and artifacts, workspace bindings, callbacks and proof
records. The runtime executes graphs and reads or writes those records.
Command execution belongs to [core](/packages/core), and memory helpers to
...
receives the request without its prompt. It returns the identity the engine
will run under: an `EngineSelectionRecord` with adapter, adapter version,
provider, model family, model, executable and capabilities. When the runtime
passes the identity it saved earlier, an engine that would run as
something else refuses. An engine without `admit` is `unsupported` for the
check, and the plan says whether that blocks the run.

A command-line adapter is admitted by path and version, not by a hash of
the executable. It doesn't control grandchildren it can't see. An API-key
adapter is admitted locally, with no executable.
...
An API-key adapter's retry layers are off for this check only.

For a command-line adapter that needs an absolute executable, use
`resolveCommandExecutable('opencode')` from `@obversa/core/command`. It reads
your `PATH` when the file runs, so a copied example doesn't contain a
machine-specific path.

## Run one

This file runs a graph whose engine isn't ready. It shows the check pause
the run before any dispatch, then the resume once the engine is ready. The
engine implements both `admit` and `run`, and it's in
...
The [graph executor](/graphs/executor) page shows what it printed.
...
| `modelIdentity` | The provider and model family a harness reports for the model it was given. Every harness that runs other providers' models, and the review gate that reads recorded models back, derives the identity through this function. |
...
input names it, the provider. `provider/model` supplies both, and a bare
model supplies only its family. The family is the lowercase part before the
...
`MEMORY_ROOT` is `/memories`, and every memory path starts there. Commands
are `view`, `create`, `str_replace`, `insert`, `delete` and `rename`.
...
this contract, and the [plugins page](/plugins) lists them.
