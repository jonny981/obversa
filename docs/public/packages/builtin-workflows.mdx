...
and `test`. The `TestCommand` record has a `command`, its `args` and an
optional `timeoutMs`. A `ReviewerSeat` has a `name`, a `seat` and an
...
has the complete example and field reference. `@obversa/builtin-workflows`
doesn't export these builders.
...
an invalid decision. The package re-exports both from
`@obversa/runtime/workflow-support`.

## Offline example

This file runs the writer, a real Node check and the reviewer offline. The
scripted engines write and inspect one temporary file. The file checks the
outcome and removes its workspace.
