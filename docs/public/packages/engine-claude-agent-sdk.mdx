...
- In the `read` and `none` workspace modes the adapter passes Claude a bounded
  tool list (`Read`, `Grep` and `Glob` for `read`; none for `none`). It
  disallows `Bash`, `Edit`, `Write`, the sub-agent tools and every MCP tool. A
  settings file outside the run cannot restore them.
