...
returns a decision and annotations tied to lines in the diff. Its
`obversa-review` command writes the result as framed JSON on stdout, and
...
The default reviews working-tree changes. `--staged` reviews the index, and
...
including a decision that requests changes. Exit 1 means the session didn't
complete or the command failed. Exit 2 means the arguments were invalid.
...
This file parses a fixed Git diff and checks the line numbers and changed
text. It runs offline and doesn't open a browser. The command above adds the
