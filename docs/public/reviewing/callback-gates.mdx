...
An approval() gate waits in one of two ways, and the record is the same in
both. A team review that uses a callback gate is not one of them: it returns a
pause carrying its question straight away, whichever way the run was started.

Attended: a person is at the run, so the process stays up and waits for the
answer. This is the mode when `onCallback: 'wait'` is set. It is never the
default: a run exits unless you ask it to wait, whichever callbacks client
it is given.

Unattended: nobody is attached, so the run records the question and exits. A
schedule starts the same file again with `resume: true`. The run reads its
record first. If the answer has arrived, the step continues. If it has not,
the run observes the pending gate and exits again with the same recorded
pause, and asks nothing a second time. The answer arrives through the stored
callbacks client.

The difference between the modes is only whether the process stays up.
...
The example, byte for byte from `examples/callback-gate.ts`. Copy it
...
For the common case, one question in a workflow, `approval` wraps the gate:
it asks through the run's callbacks client, a yes passes the step, a no goes
back to the step that owns the fix with the person's note as the finding,
and no answer pauses the run with the request pending. [A person decides](/workflows/approval)
shows it in a running file. The client, the routers and the stored history
below are what it stands on, and what a host uses to answer.
...
- **Change the question and it is a new question.** The question text, the
  ...
  does not.
- **Resume is explicit by default.** An answer arriving does not restart an
  exited run by itself; the host reads the decision and resumes the step.
  That is what lets a host decide that an approval needs a second look. With
  `onCallback: 'wait'` the process never exited, so it continues once the
  answer arrives.
