...
definitions. The factory term for that grouping is a plant: workflows that
share a domain. It is a way to describe the team's work, with no separate
...
This is the assembly pattern from a factory: earlier work supplies the
inputs for the next process. Your code makes the connection. A node can
run a child graph through the public executor and return its result.
