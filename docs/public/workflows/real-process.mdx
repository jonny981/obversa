...
```json
{
  ...
  "data": {
    ...
    "plan": {
      "status": "pass",
      "summary": "**Plan delivered** to `team-output/real-process/plan.md`.\n\n**What you get:** One task with 7 bite-sized steps (write test → verify failure → implement → verify pass → commit), following TDD. Includes exact commands to run, expected output at each step, and 5 acceptance criteria t",
      "data": "**Plan delivered** to `team-output/real-process/plan.md`.\n\n**What you get:** One task with 7 bite-sized steps (write test → verify failure → implement → verify pass → commit), following TDD. Includes exact commands to run, expected output at each step, and 5 acceptance criteria that prove the module works."
    },
    "plan-review": {
      "status": "pass",
        ...
        "results": [
          {
            ...
            "name": "architecture",
            "met": true,
            "reason": "**Completion Report**\n\nDelivered `src/result.mjs` exporting `result = 23` with passing Node test.\n\n**Process shape:** Single atomic unit. No stage boundaries — this is the minimal useful shape. If extended, future phases would build additively: same export, relaxed validation, or",
            "scope": "process shape"
          },
          {
            "kind": "verdict",
            "name": "correctness",
            "met": true,
            "reason": "## Complete\n\n- **Module:** [src/result.mjs](/private/tmp/real-process/src/result.mjs:1) exports `result = 23`.\n- **Test:** [test/result.test.mjs](/private/tmp/real-process/test/result.test.mjs:5) uses Node’s built-in test runner.\n- **Evidence:** Both tests pass: 1 test, 0 failure",
            "scope": "evidence"
          }
        ],
        "passed": 2,
    ...
    "implementation": {
      ...
      "data": "Completed in the supplied workspace.\n\n- [result.mjs](/private/tmp/real-process/team-output/real-process/src/result.mjs) exports `result = 23`.\n- [Node test](/private/tmp/real-process/team-output/real-process/tests/acceptance.test.mjs) passes: 1/1."
    },
    "conformance": {
      "status": "pass",
      "summary": "`node` exited 0"
        ...
        "results": [
          {
            ...
            "name": "architecture",
            "met": true,
            "reason": "## Process Shape Verified\n\n**Implementation complete.** All stage boundaries passed:\n\n**Stages:**\n1. **Preconditions** → Node v25.5.0 available\n2. **Plan** → TDD implementation plan with 7 steps\n3. **Build** → `src/result.mjs` exports `23`, test passes\n4. **Boundary** → Dependenc",
            "scope": "process shape"
          },
          {
            "kind": "verdict",
            "name": "correctness",
            "met": true,
            "reason": "Completed and verified.\n\n- [src/result.mjs](/private/tmp/real-process/src/result.mjs) exports `result = 23`.\n- [test/result.test.mjs](/private/tmp/real-process/test/result.test.mjs) proves the value with Node’s built-in test runner.\n- Primary and acceptance tests: 1 passed, 0 fai",
            "scope": "evidence"
          },
          {
            "kind": "verdict",
            "name": "adversary",
            "met": true,
            "reason": "Delivered:\n\n- [src/result.mjs](/private/tmp/real-process/src/result.mjs:1) exports `result = 23`.\n- [test/result.test.mjs](/private/tmp/real-process/test/result.test.mjs:5) proves the value with Node’s built-in test runner.\n\nVerification: `1 passed, 0 failed`.\n\nFailure path found",
            "scope": "failure paths"
          },
          {
            "kind": "verdict",
            "name": "conformance",
            "met": true,
            "reason": "## Completion Report\n\n**Deliverable verified:** `src/result.mjs` exports `result = 23` with passing test.\n\n**Contract compliance:**\n- ✅ Module exists at `src/result.mjs` with ESM export\n- ✅ Value is exactly `23` (verified by test and direct import)\n- ✅ Test passes: `node test/res",
            "scope": "public contract"
          }
        ],
        "passed": 4,
    ...
    "learning": {
      "status": "pass",
      "summary": "## Completion Report\n\nAll tasks completed successfully:\n\n**Module delivered:** `src/result.mjs` exports `result = 23`\n\n**Test verified:** Node test at `test/result.test.mjs` passes (1 test, 0 failures)\n\n**Learning documented:** Complete output written to `team-output/real-process",
      "data": "## Completion Report\n\nAll tasks completed successfully:\n\n**Module delivered:** `src/result.mjs` exports `result = 23`\n\n**Test verified:** Node test at `test/result.test.mjs` passes (1 test, 0 failures)\n\n**Learning documented:** Complete output written to `team-output/real-process/learning.md` covering the file structure, testing approach, and key findings for future sessions\n\nThe files already existed from a previous run. Test execution confirmed the module works correctly."
    },
    "close": {
      "status": "pass",
...
| boundary | Run the dependency and formatting checks. | Every boundary check exits 0. |
| build | Build the package and record the result. | The build exits 0. |
...
- **The action against the target is observed and reverted in the same
  ...
  change behind.
- **Every reviewer lens is a different model family from the writer it
  reviews.** The panels are the second opinions; the writer never grades
  its own work.
...
for the panels, and command gates between every inference.
