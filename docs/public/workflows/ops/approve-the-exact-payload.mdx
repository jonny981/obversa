...
description: "A person's yes is recorded against the digest of the request's bytes; the same bytes are sent, and a payload that differs by one field asks again."
---
