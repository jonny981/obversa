...
A forge is a code host such as GitHub. This helper ships a reviewed change
through a forge. It pushes the work branch, opens or updates one pull
request, proves that the change can merge, squashes the work into the base
branch, and deletes the work branch.

Call this helper after a review gate passes. A host supplies the five
...
Copy `examples/packages/forge-helper.ts` from the repository.
