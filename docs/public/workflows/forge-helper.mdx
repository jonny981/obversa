...
file shows that step for GitHub, or any host with the same five operations.
It pushes the work branch, opens or updates one pull request, proves that the
change can merge, squashes the work into the base branch, and deletes the work
branch.

Call it after a review gate passes. A host supplies the five operations, the
same way it supplies memory. The file is host-side code.
...
can merge. Every other case returns `RESULT: FAIL` with the reason on the
same line. A check run counts as passed only when its conclusion is exactly
`success`. Green checks on other workflows don't ship a change, and neither
do green checks from an earlier revision.
...
Copy `examples/forge-helper.ts` from the repository. It imports no package and
runs offline.
...
Run it with `pnpm exec tsx forge-helper.ts` in any project that has
[`tsx` installed](/get-started/installation). The result is:
...
Each of the four fixture pull requests fails the gate with its reason, as
`verdicts` shows above. The demo ships only on the exact string
`RESULT: PASS` and stops on anything else. It writes no files and makes no
network calls, so you can run it again from the start at any time.

## The GitHub adapter

The adapter returns one `gh` argument list for each operation. Each list is a
pure function of its input. A new branch uses `createRefArguments`, and an
open pull request uses `listArguments` and `editArguments`.

`runGh` is the execution layer: one function with no logic of its own. It runs
`gh` with an argument list and returns the output. It needs a `gh` login, and
nothing in this file calls it.

To use it in a process, give your host the five operations. The mock host in
the source shows each one, and the gate stays the same for every host.
