...
- **The analyse step writes the note and nothing else.** If the analyse seat
  writes the implementation itself, the step fails by name, because a plan
  written after the code is not a plan.
