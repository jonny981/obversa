...
The team works the way a real team delivers a feature. Research comes
first, with each note reviewed, then a reviewed plan and tests before code.
The implementation runs again with the reviewers' findings until the tests
pass and the panel accepts. Then a person is asked, and the last stage
writes the evidence. Inference sits at the leaves, and everything between
the leaves is a check the run can hold to. The whole team is one file that
reads top to bottom.
...
question on the record, because a person answers it, not the file. That's
where a team hands over. Everything before it ran on real engines. Use a
stored callbacks client to carry the answer into the next run.
[A person decides](/workflows/approval) explains how.
...
directory. The last stage writes the evidence and the learning after the
person's answer.
...
rejected review sends the work back to the stage it names. Each stage that
...
it, the reviewers' decision decides. A model's report of its own work
never passes a stage by itself.

`writes` names the files a stage may write. A stage that changes any other
declared file fails by name, so `tests-first` can't write the
implementation. Its tests may be red until `implement` turns them green.

Keep each reviewer and the writer it reads in different model families.
Here the Claude seat writes the research notes and the plan, and a Codex
seat reviews them. The Codex seat writes the tests and the code, and a
Claude seat reviews them. The workflow is refused before any model runs if
a panel shares a family with the stage it reviews.

A reviewer's decision is its reply. A reply with no decision is asked for
once more. Then the run pauses with an engine error, not a rejection of the
work.

A writer sent findings must rewrite its note. If the same bytes come back,
the stage ends with a plain summary instead of a second identical review.
