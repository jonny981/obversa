...
could pick up. Each story has its acceptance checks and the ticket it came
from, and a model from another family reads it back against the raw
tickets. Beside the stories are the questions to settle before anyone
writes code. Then the person who owns the product ranks them. Nothing here
writes code. The work is deciding what's worth writing, and the run stops
at the person.
...
the brief and the sample inputs from `examples/use-cases/engineering/` beside
the copied file before you run it.
...
Every event prints as one line as it happens, usage lines included. The
outcome prints last, as JSON.

The run stops at `rank` with the question pending. The stories and the
questions are in `backlog/` for the owner to read. A no goes back to `split`
with the owner's note as the finding.

Priority is the owner's call. The brief tells the models not to decide it,
so any story order in `stories.md` would only be a model's guess.

## What a run printed

The output below is from one real run of this file, with the Claude and Codex
CLIs signed in. The transcript is trimmed at both ends. Your own run prints a
`▸ run` line before the first stage. After the last stage it prints a `◂ run`
line with the outcome and the run's token total, unless the process is killed
first. This run didn't finish, and that's why it's here.

The reviewer sent the stories back three times, each time with a real
problem. One was a dark-mode story that said every screen renders "correctly"
without saying which screens or what proves it. Another was an export story
that had quietly become two: finishing a yearly export, and running one every
month. Each round the writer fixed what it was told, and the reviewer found
another real problem. On the third round the loop reached its allowance and
stopped.

Nothing reached the owner. A grooming loop that stops with the work
unfinished is doing its job. One that runs out of attempts and passes the
stories on anyway isn't. Match the allowance to the work. Three rounds suit a
job with a few defensible answers, and splitting a backlog of vague tickets
isn't that job.
...
The "done when" column holds each stage's `gate`. The writing seat reads
these sentences in its prompt, but the package doesn't check them. It checks what each stage
declares: a file in `writes` that's missing or empty fails the stage, a
reviewed stage passes only when its reviewer accepts, and a person stage
pauses until the person answers. So a model's own report of its work never
passes a stage here.

A raw ticket with no story sends the split back, because the reviewer reads
the raw tickets as well as the stories. That's what happens in
`backlog-groom-then-rank.proof.ts`. A split sent back with findings must
change. If the same bytes come back, the stage ends instead of running a
second identical review.

## Source

The file is `examples/use-cases/engineering/backlog-groom-then-rank.ts`. Its
proof, brief and sample inputs sit beside it. The
[runtime package page](/packages/runtime) lists every stage and workflow key.
