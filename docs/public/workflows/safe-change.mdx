---
title: "A file change approved byte for byte"
description: "Approve exact file bytes, keep a complete backup, and reconcile an interrupted write."
---

This example combines protected source records into destination files.
...
It uses four synthetic source kinds: document, current record, discussion
comment, and historical entry. Each complete source record, with its metadata
and exact body text, is one protected fact. The proposal and the review are
scripted, and no model is called.
...
refused. The example doesn't extract facts from prose or decide which facts
can be discarded.
...
The fixture's `approve()` function supplies a scripted answer. A real host
must collect its decision through the stored callback client.

The backup keeps the original source and target file bytes, including every
record field needed to restore them. Artifact hashes protect these bytes.
The example checks the backup before use. There's no automatic restore.
...
The example appends intent and result events to each target's own stream.
...
is lost, the example reads the target before deciding whether it completed.

After a process crash leaves an action's outcome unknown, the executor
pauses that attempt for reconciliation. On resume, the example checks the
...
The adapter assumes one writer owns the synthetic record directory.
An external service needs its own conditional-write and action-status
contract. The example makes no guarantee about any remote API.
...
The example is three files that sit side by side.
`safe-change-recipe.ts` drives the stored graph,
`safe-change-file-adapter.ts` reads and changes records, and
`safe-change.ts`, shown below, runs the fixture. Keep them together. Apart
from each other and Node's built-in modules, they import only public runtime
exports.
...
The test suite compiles all three files against the packed runtime with
TypeScript 6 and TypeScript 7. It runs both the compiled files and the
TypeScript sources, and checks their report against this page.
