...
description: "Every clause mapped to the playbook rule it meets or breaks, redlines in the wording the playbook allows and checked by a second model, and a negotiating note. A lawyer decides what is sent."
---

The brief is the playbook: what you accept, what you push back on and
with what wording, and what you never sign. One model maps every clause of
...
family checks each redline against the playbook. It sends the set back
...
the brief and the sample inputs from `examples/use-cases/business/` beside the
copied file before you run it.

## The file

Write every rule you want applied in `briefs/playbook.md`. Everything a
seat knows about what's acceptable comes from that file. Only the rules
written there are applied.

`files: ["contracts/msa.md"]` in the brief's front matter names the contract
as a workspace file. It tells every seat the file exists and forbids the
`review` seat from writing it.
...
CLIs signed in, trimmed at both ends. The first seat on that machine also had
a local search tool that this file doesn't configure. Those calls are cut, and
your own run won't make them.
...
| positions | Write the negotiating note: what to hold, what to concede and to what, and what is a walk-away. | Every redline has a position, and a reviewer from another family has accepted them. |
| negotiate | Put the redlines and the positions in front of the lawyer. | The lawyer has decided. |

The package doesn't check the "done when" sentences itself. `clauses` has no
reviewer, so only its own seat judges whether every clause is on the list.
[The keys of a stage](/packages/runtime#the-keys-of-a-stage) list what the
package does check.

## Source

The file is `examples/use-cases/business/contract-playbook.ts`. Its proof,
brief and sample inputs sit beside it. The
