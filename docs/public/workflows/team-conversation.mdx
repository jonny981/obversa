...
`TEAM_NODE_FAILED` reports a failed member. `TEAM_TURN_LIMIT` reports a
queued member whose turn cap is exhausted. A paused member keeps the run
paused until the host uses the executor's existing resume contract.
