...
Copy `examples/workflows/feature-delivery.workflow.ts` from the repository.
...
`tsx` installed, `pnpm exec tsx feature-delivery.workflow.ts` runs the same workflow.
