...
4. The `review` stage asks three model seats to read the source and tests.
   ...
   `implement` within the feedback-loop budget.
...
| Limit | Two returns through the feedback loop |
...
The feedback loop allows two returns. If the panel keeps rejecting the work, the
