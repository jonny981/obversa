...
This file takes one change from a written request to an approved result
with no model account. The first draft passes its own tests but
misses a requirement. The review panel catches the miss and sends it back.
The workflow repairs the work, proves it again, and approves the exact final
bytes.

## Graph

1. The `analyse` stage turns the issue into typed acceptance criteria
   before any code runs. Work with no accepted criteria fails the workflow.
...
4. The `review` stage is a panel of three reviewers, and two votes pass it.
   ...
   `implement` as a kickback, bounded by the workflow's kickback budget.
...
6. The `approve` stage binds the approval to the sha256 of the final
   source. The request, the claim, the answer and the digest are durable
   events in an approval record beside the run record. The stage reads the
   answer, and a no fails the workflow. The small approval graph in the
   source exists so the stored callback client can open its record; it is
   not the workflow's own graph.
...
| Permissions | Writes only inside its own temporary directory. The test stage starts one `node --test` child process that inherits the run environment. No network access |
| Limit | Two kickbacks |
...
The workflow reads its own record back before it exits. `implementRuns` and
...
record rather than from memory. The approval answer lands in its own
durable record beside the run record.

After you copy the source into a project that has `@obversa/runtime` and
`tsx` installed, `pnpm exec tsx feature-delivery.ts` runs the same workflow.
...
with code 1. A refused approval fails the workflow the same way: the approver
who answers no stops the ship, and the no is a durable event in the
approval record. The workflow never advances past a failing test command, so an
unproven draft cannot reach the panel or the approval.
...
`fnJob` with `agentJob` and an engine, and the same workflow drives a model that
writes the change; replace the panel's reviewer jobs with `agentCheck`
conditions and models read the code; put a person behind the approval gate
in place of the scripted submit, answering through the same claim and
digest protocol against the same record. The stages, the kickback, and the
