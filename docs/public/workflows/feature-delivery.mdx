...
description: "Take one written issue through analysis, implementation, a real test run, a review panel, a feedback loop, and a person's approval."
---
...
result. A command gate runs the tests before the review panel. A rejection
sends the work back to `implement`, at most twice.
...
5. The `approve` stage pauses for a person. A refusal sends the work back to
   `implement` with the person's note.
...
| Evidence | One JSON result on standard output and one JSONL run record under `.obversa/records/` |

## Source

Copy `examples/feature-delivery.ts` from the repository. It imports only public
package exports and takes its Claude and Codex seats straight from the engine
packages, so it needs no other local file.

Set up the project as [Installation](/get-started/installation) shows. The
bundle brings the runtime and both engine plugins:
...
`pnpm exec tsx feature-delivery.ts` runs it. The result is:
...
`paused` means the run stopped at `approve` with its question pending.
`recordTo: 'auto'` writes the event record under `.obversa/records/` in the
run's workspace.

## Failure behavior

The feedback loop allows two returns. If the panel keeps rejecting the work, the
budget is spent and the review's failed verdict stands. A refused approval also
sends the work back, and fails the workflow once the budget is spent. The
workflow never advances past a failing test command.
...
implementation and test review. The command gate is a local check, and the
person gate is a callback the host handles.
