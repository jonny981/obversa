---
title: "Examples"
description: "Teams you can run on your own work today, inside and outside software delivery, and the single mechanisms they are built from."
---
...
Every one of these ends at a person, so a run stops before anything is
sent or filed; each is a complete file with its brief and sample inputs
beside it under `examples/use-cases/`.
...
- [Architecture, then a spike, then the team decides](/workflows/architecture-then-spike).
  Research and a spec, each reviewed; the smallest program that proves
  the risky part, which has to run; a panel of two other families; a
  person says build it.
- [Watch product signals, file the work](/workflows/watch-signals-then-file).
  A week of rage clicks, exceptions and form errors pulled from PostHog,
  grouped and checked, written as tickets; a person decides what is filed.

Business and commercial:

- [Draft, then a person sends](/workflows/draft-then-send). Emails
  researched and drafted, read for tone and for claims we cannot back;
  the person whose name is on them approves the exact payload a command
  sends.
- [Support triage with a confidence gate](/workflows/support-triage). A
  route and a confidence, checked by a second model; a confident reply
  goes out by command and an unsure one goes to a person.
- [Contract review against a playbook](/workflows/contract-playbook).
  Every clause mapped to the rule it meets or breaks, redlines in the
  playbook's wording and checked, a negotiating note; a lawyer decides.

Other:

- [Translate, reflect, glossary, then you](/workflows/translate-reflect).
  A translation read back as an editor would and sent back with what to
  change, every glossary term accounted for; the person who knows the
  readers decides.
