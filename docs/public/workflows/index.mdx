...
- [Feature delivery](/workflows/feature-team). Eleven steps: research and
  a plan, each reviewed; tests before code; an implementation that repeats
  with the reviewers' findings; verification, approval, evidence. Work
  goes back to the step that owns it.
- [The shape of a real process](/workflows/real-process). Twenty-three
  stages in one file, inference only at the leaves: preconditions, research
  with reviews, a plan a person can see, tests first, an implementation
  loop, conformance, build, a promotion gate, an observed and reverted
  action, deployed checks, a CI watch, evidence and learning. It runs as
  far as your machine allows and records every stage it skipped, with the
  reason.
