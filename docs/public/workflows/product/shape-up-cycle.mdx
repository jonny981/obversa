...
description: "Pitches, a person's bets, and a build inside a fixed time."
---
...
a fixed time, with the scope cut to fit the time rather than the time
...
record, and the same shape holds for a whole organisation, one cycle at a
time.
