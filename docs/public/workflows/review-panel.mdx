...
process completed without a transport failure, but its response was a tool
request rather than the required JSON decision, so the panel records no
decision as a non-blocking engine error. With `agree: 1` the change passed
on the one acceptance. Set `agree` to two and the same run would have failed
the panel, because an error is not an acceptance.
