...
- **The question is about what came before.** By default the request's
  ...
  upstream after a feedback loop returns work is a new question and the same question about
...
- **A question nobody can answer fails plainly.** After asking, the step
  ...
  the same question after a feedback loop returns work, never gets there.
...
- **Running again runs the earlier steps again.** `run` starts a fresh
  ...
  the question finds its answer. The supervised runner resumes a paused
  run at the step it stopped on, but it drives a graph definition through
  a host module, not a file like this one; see
