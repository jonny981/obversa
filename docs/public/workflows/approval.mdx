...
- **The client is the run's.** `run` takes `callbacks`, the in-memory
  ...
  as `ctx.callbacks`. The default is one in memory that lives for the
  process. Pass the stored client over a directory and a paused run finds
  its answer after a restart: the next run over the same store reads it
  from the history.
