...
This file tries the Git workspace provider without touching any repository
you care about. It makes its own temporary repository, forks a child worktree
from it, and deletes both at the end.
...
4. **Child state:** It reads the child `HEAD` and `README.md`. It throws if the
   revision or file contents differ from the captured repository.
...
The file prints one JSON object with three fields:
