...
The cmux glue is not an npm package: it is five small scripts and one
Command Palette entry. One command installs them.

```bash
curl -fsSL https://obversa.ai/install/cmux.sh | bash
```

It writes only into `~/.obversa/cmux`, prints every path before it writes,
and never edits an existing cmux configuration: if you already have one it
copies it aside and prints the entry for you to add. macOS only today,
because the default-browser fallback uses `open`.
...
setup rather than a product, and there is no host fork and no new UI.
...
If you would rather do it by hand, the scripts are in `hosts/cmux/bin` in
the repository, and the palette entry names the launcher by absolute path.
