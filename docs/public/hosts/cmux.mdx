...
The URL a surface opens carries the session's bearer token, so every
command the glue runs (`cmux`, `open`, `xdg-open`, and the `python3`
that reads the cmux tree) runs by absolute path from a fixed list of
system directories (`/usr/bin`, `/bin`,
