...
This page is for people working on Obversa itself. The cmux glue is not a
published package: it is configuration and a few small scripts that live in
the repository, so it needs a checkout, which nothing else here does. When
the host ships as a package this page changes with it.
...
The scripts live in `hosts/cmux/bin`. Call them by full path, or put that
