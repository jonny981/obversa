...
identity when available. Without a report, it uses the selected identity for
routing. Stored facts and later routing use the same provider and model key.
Later dispatches skip either identity. If every declared target is dead,
