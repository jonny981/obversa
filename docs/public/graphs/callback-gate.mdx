...
2. **Claim:** `claim` assigns it to exactly one router and returns a private
   claim token.
...
4. **Release:** `release` returns a claimed request to the pending list. The
   `directRouter` helper also releases the claim if its responder throws.
...
The built-in validator checks the plain response shape: required top-level
fields and the declared top-level field types. It is not a full JSON Schema
validator. A host that needs nested constraints must validate them before it
submits the answer.
