...
After a dispatch is recorded in the event history, `decide` must not return
that request again. A later event can make the same node dispatchable again;
that later occurrence is a new request.
