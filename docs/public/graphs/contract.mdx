...
Copy `examples/custom-graph.ts` when you start a graph type. The
...
A form may also return the pure optional operation
`validateNodeResult(nodeId: NodeId, result: JsonValue): GraphValidationIssue | null`.
Before saving a completion, the executor calls it with a declared node ID and
copied, frozen result data. `null` accepts the result; an issue with string
`code`, `path` and `message` fields records `node-failed` with `RESULT_INVALID`.
`compileGraph` checks and freezes the returned issue. A malformed return or an
exception from the implementation propagates as an error. Forms that omit the
...
task and separate answers. It does not use the stored graph executor or
acquire replay by being passed to `run()`. For saved room messages and
turns requested by mentions, compile `teamGraphType` as shown in
[Team conversation](/workflows/team-conversation).
...
In a Git workspace, each agent receives the task and its standing brief in
a separate worktree through `isolated()`. Passing changes merge into the
caller's branch under the existing merge lock. Outside a Git repository,
that wrapper warns and runs in the shared workspace. A passing team result
is therefore not proof that a Git merge occurred. No second merge or lease
mechanism is added.

The callable team returns each member's separate answer. `review` reuses an
existing review panel or callback gate. Each review keeps that panel's or
gate's own contract.
...
preserved. An integration conflict returns `fail`.
