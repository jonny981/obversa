...
and removes that worktree. Passing agent changes land on the caller's branch
through the existing merge lock. This form adds no second merge or lease
mechanism.

The form is the members working apart and answering separately. `review` reuses
an existing review panel or callback gate. Each review keeps that panel's or
gate's own contract.
