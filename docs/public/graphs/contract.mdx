...
`decide` returns one or more dispatch commands, or exactly one `pause`,
`complete`, or `fail` command. An empty decision is invalid.
...
Invalid data throws `GraphValidationError`.
