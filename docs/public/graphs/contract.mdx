...
`integrate` is omitted, the member work remains unlanded. This form adds no
second merge or lease mechanism.
...
- **Integration:** omitted `integrate` leaves member work unlanded; supplied
  `integrate` uses the caller's branch as the target and accepts the existing
  `isolated()` `onConflict` option.
