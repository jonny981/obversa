...
`team()` is a graph form for several agents working on one task. It uses the
same public graph contract as the other forms on this page. It is not a new
kernel object.
...
Worktree isolation is the behavior. Each agent receives the task and its
standing brief in a separate worktree. The existing `isolated()` path creates
and removes that worktree. When `integrate` is supplied, passing agent changes
return through the existing merge lock and land on the caller's branch. When
`integrate` is omitted, the existing `isolated()` conflict policy applies.
This form adds no second merge or lease mechanism.

The form is the members working apart and answering separately. `review` reuses
an existing review panel or callback gate; those panel forms are the answering-
together members of the family. The reviewer identity rule is the one
convergence enforces. `team()` adds no second diversity rule or switch.

An existing review panel receives the completed team result as its previous
outcome, so its reviewers can inspect the member outcomes and task.

The team returns the existing `Outcome` shape. Its `data` field contains a
`TeamResult`; a review revision, failure, pause, or error remains on the outer
`Outcome` so a team can be placed in a larger graph without an adapter.

A callback review creates the request through `createCallbackGate` and returns
`paused`. The nested review outcome contains that request in its `data` field.
The host posts and answers it through the existing callback client, then
controls resumption of the paused work.

The team status is `pass` only when every agent passes, the configured review
passes, and configured integration lands. A member failure or review failure
returns `fail` with all member results preserved. A member or review pause
returns `paused` with its reason preserved. An integration conflict returns
`fail` unless the selected existing synthesis policy resolves it.

The form validates these rules before it runs:
...
- **Integration:** supplied `integrate` accepts the existing `isolated()`
  `onConflict` option; omission uses `isolated()`'s default conflict failure.
- **Review:** a configured review uses the existing panel or callback contract.

The communication extension is additive and off unless configured. Its fields
are:
...
interface TeamConfigWithCommunication extends TeamConfig {
  communication?: TeamCommunication;
}
```

When that extension is implemented, room membership is frozen with the plan,
room messages are recorded in the run log, and a readable room-file projection
is written beside the record. A mention dispatches the named role with only its
permitted room tails. A wake cue only notifies a host; it is never the record.
