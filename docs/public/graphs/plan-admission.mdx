---
title: "Plan admission"
description: "Bind a graph description to a host admission record, and freeze what the run may use: package, permissions, lanes, and the engine checks that run before the first step."
---

Use `resolveGraphPlan(description, resolution)` after you compile a graph.
It returns a frozen plan, canonical JSON, and a SHA-256 digest.

The plan is data. It can't execute code or grant a permission.

## Host admission

The host provides a `PlanResolution`. It contains:

- The graph package source, version, and digest.
- The same package identity in the host admission record.
- The permissions that the host admits.
- One effective execution target for each declared execution lane.
- An optional ordered fallback list for each lane. An omitted list means no
  fallback.
- An optional preflight policy: a `timeoutMs` for the checks that run
  before the first step, and one entry per lane with `live: 'required'`
  or `'skip'` and `unsupportedStatic: 'block'` or `'allow'`. An omitted
  policy means no checks.

The package identity in the resolution and admission record must match exactly.
The graph package asks for permissions in `describe()`. Each requested
permission must match an admitted permission by name and JSON scope. If it
doesn't match, `resolveGraphPlan` throws `GraphValidationError` with
`PERMISSION_NOT_ADMITTED`.

Extra admitted permissions don't become graph permissions. The resolved plan
keeps requested and admitted permissions as separate lists.

When a preflight policy is present, every resolved lane needs exactly one
entry, and no entry may name a lane that doesn't exist. `timeoutMs` must be a
whole number of milliseconds from 1 to 2147483647. `resolveGraphPlan` rejects
a policy that breaks either rule before it freezes the plan.

## Resolve execution lanes

Each described lane names a requested adapter, provider, model family, model,
and tools. A graph can also declare known substitutions.

The host resolves every described lane exactly once. An effective target must
equal the requested target or one declared substitution. `resolveGraphPlan`
rejects unknown, missing, or extra lane resolutions.

The resolved plan records the requested target, effective target, and fallback
order. A change to an adapter, provider, model family, model, tools, allowed
substitution, fallback order, or preflight policy changes the plan digest.

## Plan snapshot

Resolve the plan before a host starts work. The returned plan snapshot is
frozen. Its package identity, permissions, lanes, policies, preflight policy,
and bounds stay in the snapshot that its digest identifies.

Run parameters aren't part of the resolved plan or its digest. The run record
keeps resolved inputs as part of the frozen run.

## Run parameters

`run(job, { params })` accepts a JSON object. If `params` is `undefined`, the
run uses a frozen empty object. `run` rejects `null`, arrays, and other
non-object values with `JsonValueError` before work or environment setup
starts.

For a valid object, `run` clones and freezes `params` before work starts. The
root job and each child job receive the same frozen object as `ctx.params`. A
later change to your `params` object can't change `ctx.params`. A nested value
that isn't JSON also throws `JsonValueError`, and the error's `path` points to
that value.

## Trusted package boundary

An [outside graph type](/graphs/contract) is trusted package code. The
conformance kit isn't a security sandbox.

The host owns package discovery, signature checks, and sandboxing. The graph
contract only validates the package identity, admission record, and plan data
that the host provides.

## Limits

Plan admission doesn't schedule nodes, store runs, provide built-in graph
forms, or execute work on another machine.
