...
description: "One row per tool Obversa ships or drives: the six engines, the two memory adapters, the cmux host, and Plannotator."
---
...
thirteen package pages under [Packages](/packages/runtime) stay the
reference for every package.
...
- **Grok CLI.** `@obversa/engine-grok-cli` runs engine requests through a
  ...
  CLI 1.0.5, and host-selected authentication.
...
- **In-process memory.** `@obversa/memory-simple` stores memory in one
  process. A reader needs nothing beyond the package.
...
- **The cmux host.** The cmux glue opens the local web surface and the
  Plannotator page as terminal splits, and opens one editor window with
  the two Order workspace roots. A reader needs cmux installed. See
