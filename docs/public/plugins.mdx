...
Each engine plugin runs one bounded request through a CLI, SDK or API. Install
the bundle first:
...
The CLI engines also need their command line tools installed and authenticated.
Jev is installed separately; choose it for structured assessments, not workspace
coding:
...
- **Codex CLI.** `@obversa/engine-codex-cli` runs engine requests through a
...
- **Claude Agent SDK.** `@obversa/engine-claude-agent-sdk` runs engine requests
...
- **Jev API.** `@obversa/engine-jev-api` answers structured questions over
  recorded state. A reader needs Node.js 22.12 or later, a Jev endpoint and
  an API key. Use workspace mode `none`; it does not read or edit workspace
  files. Routing and thresholds remain workflow policy. See
  [The Jev engine](/packages/engine-jev-api).
...
- **Markdown search.** `@obversa/memory-markdown` selects ranked passages
  from a local Markdown directory, then exposes the selected files to
  `ground` through a read-only Memory view. A reader needs Node.js 22.12 or
  later. Home: [The memory-markdown package](/packages/memory-markdown).
...
- **Webhook notifications.** `@obversa/notify-webhook` posts one message per
  ...
  finished, a review sent work back with the reason, the run is waiting for a
  person, and how it ended. The body carries a `text` field, so a Slack,
  Discord or Teams incoming webhook renders it with no code of its own. A
  reader needs Node.js 22.12 or later and a URL to post to. Home:
